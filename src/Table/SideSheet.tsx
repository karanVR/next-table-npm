...
const SideSheet = ({ data, isLoading }: { data?: any; isLoading?: boolean }) => {
  return (
    <div className={(cn('h-full '), data ? '' : 'grayscale')}>
      ...
      {!isLoading && (
        <div className={cn('md:bg-[#F3F5FD] flex flex-col relative h-[74dvh] ')}>
          <div className="flex justify-between p-4 items-center relative top-1">
            ...
            <div
              className={cn(
                ...
                data?.status === 1 ? 'text-green-500' : data?.status === 4 ? 'text-yellow-500' : '',
              )}
            >
              <FaCircle size="8px" />
          ...
          <div className="p-4 flex flex-col gap-2 ">
            <TextSmall className="inline font-bold text-sm ">Contact Information</TextSmall>
            <TextSmall className="inline  text-[10px] text-[#7E7E7E]">
              Name:{' '}
              <TextSmall className="inline text-black text-xs">{data?.customer?.name}</TextSmall>
            </TextSmall>
            <TextSmall className="inline text-[10px] text-[#7E7E7E]">
              Phone Number:{' '}
              <TextSmall className="inline text-black text-xs">{data?.customer?.phone}</TextSmall>
            </TextSmall>
            <TextSmall className="inline  text-[10px] text-[#7E7E7E]">
              Email ID:{' '}
          ...
          <Separator />
          <div className="p-4 flex flex-col gap-2">
            <TextSmall className="inline font-bold text-sm color-[#7E7E7E]">Plan Details</TextSmall>
            <TextSmall className="inline  text-[10px] text-[#7E7E7E]">
              Plan Name:{' '}
              <TextSmall className="inline text-black text-xs">{data?.display_plan_name}</TextSmall>
            </TextSmall>
            <TextSmall className="inline text-[10px] text-[#7E7E7E]">
              Plan Serial Number:{' '}
              <TextSmall className="inline text-black text-xs">
            ...
            <TextSmall className="inline  text-[10px] text-[#297FFF] font-bold">
              Plan MRP:{' '}
              <TextSmall className="inline"> {data?.kitprice?.customer_price ?? 1999}</TextSmall>
            </TextSmall>
            <TextSmall className="inline  text-[10px] text-[#297FFF] font-bold">
              Retailer Landing Cost:{' '}
              <TextSmall className="inline"> {data?.kitprice?.retailer_price ?? 1099}</TextSmall>
            </TextSmall>
          </div>
          <Separator />
          <div className="p-4 flex flex-col gap-2">
            <TextSmall className="inline font-bold text-sm ">Product Details</TextSmall>
            <TextSmall className="inline  text-[10px] text-[#7E7E7E]">
              IMEI Number:{' '}
              <TextSmall className="inline text-black text-xs">
            ...
            <TextSmall className="inline text-[10px] text-[#7E7E7E]">
              Model:{' '}
              <TextSmall className="inline text-black text-xs">{data?.items[0]?.model}</TextSmall>
            </TextSmall>
            <TextSmall className="inline  text-[10px] text-[#7E7E7E]">
