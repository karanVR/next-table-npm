...
export const columns: ColumnDef<SamsungColumns>[] = [
  {
    ...
    // @ts-expect-error
    header: (
      <TextSmall className="text-sm text-[#4C4D4E] font-SamsungSharpSans font-bold">
        S.No
      </TextSmall>
    ),
    cell: ({ row, index }: any) => {
  ...
  },
  {
    ...
    cell: ({ row }: any) => {
      ...
      return (
        <TextSmall className="font-light">
          {row.original.items[0].imeinumber}
        </TextSmall>
      );
    },
  },
      ...
      if (status) {
        return (
          ...
          >
            <FaCircle size="8px" />
            <TextSmall>
              {status === 4 ? "Pending" : status === 1 ? "Active" : ""}
            </TextSmall>
          </div>
        );
      }
  ...
  },
  {
    ...
    header: (
      <div className="text-sm w-[7vw] text-[#4C4D4E] ">
        <TextSmall className="font-bold font-SamsungSharpSans ">
          Plan Type
        </TextSmall>
      </div>
    ),
    cell: ({ row }: any) => {
      ...
      return (
        <TextSmall className="font-light">
          {row.original.display_plan_name}
        </TextSmall>
      );
    },
  },
